...
fromCStringPluralDefault :: (Eq a, Num a)
...
-- | getText wraps GNU gettext function. It returns translated
...
-- 'getText' but also could take domain name.
--
...
-- | nGetText wraps GNU ngettext function. It translates text string
...
nGetText :: String              -- ^ msgid in singular form
         ...
         -> IO String           -- ^ result string, by default if
                                -- number is 1 than singular form of
                                -- msgid is returned, otherwise ---
                                -- plural
...
dnGetText :: Maybe String       -- ^ domain name, if 'Nothing' ---
          ...
          -> String             -- ^ msgid in plural form
          -> Integer            -- ^ number, used to choose appropriate form
          -> IO String          -- ^ result string, by default if number
                                -- is 1 than singular form of msgid is returned,
                                -- otherwise --- plural
...
dcnGetText :: Maybe String      -- ^ domain name, if 'Nothing' ---
          ...
          -> Integer            -- ^ number, used to choose appropriate form
          -> IO String          -- ^ result string, by default if number
                                --  is 1 than singular form of msgid
                                --  is returned, otherwise --- plural
...
bindTextDomain :: String        -- ^ domain name
               -> Maybe String  -- ^ path to the locale folder or 'Nothing'
                                -- to return base directory for domain
               -> IO String     -- ^ return value
